...
function ProdusCard({
  ...
  return (
    <a
      ...
      className="group bg-[#15120c] border border-[#37301f] hover:border-[#c9a63e]/70 rounded-2xl overflow-hidden transition-all hover:shadow-[0_0_24px_rgba(184,145,46,0.22)] hover:-translate-y-1 duration-200 flex flex-col relative"
    >
      {/* Image */}
      <div className="relative bg-[#26211a] overflow-hidden" style={{ aspectRatio: "1" }}>
        {hasImg ? (
          ...
          <div className="w-full h-full flex flex-col items-center justify-center gap-2 bg-[#26211a]">
            <span className="text-4xl">🛍️</span>
            <span className="text-[10px] font-bold text-[#a89a78] text-center px-2 leading-tight">{merchant}</span>
          </div>
        )}
        ...
        <button
          ...
          className={`absolute bottom-2 right-2 w-7 h-7 rounded-full flex items-center justify-center text-sm transition-all duration-150 shadow-lg ${
            isSaved
              ...
              : "bg-[#15120c]/80 text-[#a89a78] opacity-0 group-hover:opacity-100 hover:text-red-400"
          }`}
      ...
      <div className="p-3 flex flex-col flex-1">
        <p className="text-[10px] text-[#a89a78] mb-0.5 truncate">{p.brand || merchant}</p>
        <p className="text-xs font-semibold text-[#dcd0b8] line-clamp-2 flex-1 group-hover:text-[#d8c091] transition-colors leading-snug">
          {p.title}
        </p>
        <div className="flex items-baseline gap-2 mt-2 flex-wrap">
          ...
          {p.old_price && p.old_price > p.price && (
            <span className="text-[10px] text-[#a89a78] line-through">{p.old_price.toFixed(2)} lei</span>
          )}
        </div>
        <div className="mt-2 flex items-center gap-1 text-[10px] font-bold text-[#d8c091]">
          Cumpara acum
...
function SkeletonCard() {
  return (
    <div className="bg-[#15120c] border border-[#26211a] rounded-2xl overflow-hidden animate-pulse">
      <div className="aspect-square bg-[#26211a]" />
      <div className="p-3 space-y-2">
        <div className="h-2 bg-[#26211a] rounded w-1/2" />
        <div className="h-3 bg-[#26211a] rounded w-full" />
        <div className="h-3 bg-[#26211a] rounded w-3/4" />
        <div className="h-4 bg-[#37301f] rounded w-1/3 mt-2" />
      </div>
    </div>
  );
}
    ...
    <div className="min-h-screen bg-[#0b0a07]">
      ...
      <section className="relative bg-[#0b0a07] text-white overflow-hidden">
        <div className="absolute inset-0 pointer-events-none">
          <div className="absolute inset-0" style={{ background: "radial-gradient(ellipse 80% 60% at 50% 0%, rgba(184,145,46,0.12) 0%, transparent 65%)" }} />
        </div>
        <div className="relative max-w-5xl mx-auto px-4 pt-12 pb-14 text-center">
          <nav className="flex justify-center gap-2 text-xs text-[#8c8064] mb-6">
            <Link href="/" className="hover:text-[#c8bda2] transition-colors">AmCupon.ro</Link>
            <span>/</span>
            <Link href="/produse" className="hover:text-[#c8bda2] transition-colors">Produse</Link>
            <span>/</span>
            <span className="text-[#c8bda2]">{catMeta.label}</span>
          </nav>
          ...
          <p className="text-[#a89a78] text-base mb-8 max-w-xl mx-auto">{catMeta.desc}</p>
          ...
          <div className="mt-6 flex justify-center">
            <Link href="/wishlist"
              className="inline-flex items-center gap-1.5 text-xs text-[#8c8064] hover:text-red-400 transition-colors border border-[#37301f] px-3 py-1.5 rounded-full hover:border-red-400/50">
      ...
      <section className="max-w-7xl mx-auto px-4 py-8">
        ...
        {products.length === 0 ? (
          <div className="text-center py-20 bg-[#15120c] rounded-2xl border border-[#37301f]">
            <p className="text-5xl mb-4">{catMeta.emoji}</p>
            <h2 className="font-black text-[#dcd0b8] text-xl mb-2">Feed-urile se actualizeaza</h2>
            ...
            <Link href="/produse" className="bg-[#b8912e] text-white font-bold px-6 py-3 rounded-xl hover:bg-[#c9a63e] transition-colors text-sm inline-block">
          ...
          <>
            ...
            <div className="bg-[#15120c] rounded-2xl border border-[#37301f] p-4 mb-6">
              <div className="flex flex-wrap gap-3 items-end">
                ...
                <div className="flex flex-col gap-1 flex-1 min-w-[200px]">
                  <label className="text-[10px] font-bold text-[#a89a78] uppercase tracking-widest">Cauta</label>
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full bg-[#26211a] border border-[#37301f] text-[#dcd0b8] placeholder:text-[#8c8064] rounded-xl pl-9 pr-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#c9a63e]"
                    />
                  </div>
                </div>
                ...
                {brandList.length > 1 && (
                  <div className="flex flex-col gap-1 min-w-[140px]">
                    <label className="text-[10px] font-bold text-[#a89a78] uppercase tracking-widest">Brand</label>
                  ...
                  <div className="flex gap-1">
                    {[0, 10, 25, 50, 70].map((v) => (
                      <button key={v} onClick={() => setMinDiscount(v)}
                        className={`px-2.5 py-2 rounded-xl text-xs font-semibold transition-colors ${minDiscount === v ? "bg-[#b8912e] text-white" : "border border-[#37301f] bg-[#26211a] text-[#c8bda2] hover:border-[#c9a63e]"}`}>
                        {v === 0 ? "Toate" : `>=${v}%`}
                      </button>
                    ))}
                  </div>
                ...
                {pretMax > 0 && (
                  <div className="flex flex-col gap-1 min-w-[160px]">
                    <label className="text-[10px] font-bold text-[#a89a78] uppercase tracking-widest">
                    ...
                    <input
                      ...
                      className="accent-[#c9a63e] w-full h-2 cursor-pointer"
                    />
                    <div className="flex justify-between text-[9px] text-[#8c8064]">
            ...
            <div className="flex items-center justify-between mb-4">
              <p className="text-sm text-[#a89a78]">
                <span className="font-bold text-[#dcd0b8]">{filtrate.length.toLocaleString()}</span> produse
                {hasFiltre && <span className="text-[#8c8064]"> (din {products.length.toLocaleString()})</span>}
                {updated && <span className="text-[#473d28]"> · actualizat {new Date(updated).toLocaleDateString("ro-RO")}</span>}
              </p>
              <Link href="/wishlist" className="text-xs text-[#8c8064] hover:text-red-400 transition-colors flex items-center gap-1">
                ♥ Wishlist
              </Link>
            </div>
            ...
            {filtrate.length === 0 ? (
              <div className="text-center py-16 bg-[#15120c] rounded-2xl border border-[#37301f]">
                ...
                <button onClick={resetFiltre} className="text-[#d8c091] font-semibold text-sm hover:text-[#e3d1a6]">
                  Reseteaza filtrele →
                </button>
              </div>
            ) : (
              <>
                ...
                {limit >= filtrate.length && filtrate.length > 48 && (
                  <p className="text-center text-xs text-[#473d28] mt-4">
      ...
      {faqItems.length > 0 && (
        <section className="max-w-3xl mx-auto px-4 py-12">
          ...
          <div className="space-y-3">
            {faqItems.map((item, i) => (
              <div key={i} className="bg-[#15120c] border border-[#37301f] rounded-2xl overflow-hidden">
                <button
                  onClick={() => setFaqOpen(faqOpen === i ? null : i)}
                  className="w-full text-left px-5 py-4 flex items-center justify-between gap-3 hover:bg-[#26211a] transition-colors"
                >
                  <span className="font-semibold text-[#dcd0b8] text-sm">{item.q}</span>
                  <span className={`text-[#d8c091] text-lg transition-transform ${faqOpen === i ? "rotate-45" : ""}`}>+</span>
                </button>
                {faqOpen === i && (
                  <div className="px-5 pb-5 text-sm text-[#a89a78] leading-relaxed border-t border-[#26211a] pt-3">
                    {item.a}
                  </div>
                )}
              </div>
          ...
          <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-3">
            {alteCategorii.map(([slug, cm]) => (
              <Link key={slug} href={`/produse/${slug}`}
                className="group bg-[#26211a] border border-[#37301f] hover:border-[#c9a63e]/50 rounded-xl p-3 text-center hover:shadow-md hover:-translate-y-0.5 transition-all duration-200">
                <div className="text-2xl mb-1">{cm.emoji}</div>
                <p className="text-xs font-semibold text-[#c8bda2] group-hover:text-[#d8c091] transition-colors leading-tight">{cm.label}</p>
              </Link>
            ))}
          </div>
          <div className="mt-4 text-center">
            <Link href="/produse"
              className="text-sm font-bold text-[#d8c091] hover:text-[#d8c091] border border-[#c9a63e]/30 px-5 py-2 rounded-xl bg-[#15120c] inline-block">
              Toate produsele ({totalAll.toLocaleString()}) →
            </Link>
          </div>
        </div>
