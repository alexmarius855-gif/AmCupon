...
function ProdusCard({ p, onSave, saved }: { p: Produs; onSave: () => void; saved: boolean }) {
  ...
  return (
    <div className="group bg-[#ffffff] border border-[#cbd5e1]/60 hover:border-[#14b8a6]/60 rounded-xl overflow-hidden transition-all hover:shadow-xl hover:shadow-[#14b8a6]/5 hover:-translate-y-1 duration-200 flex flex-col relative">
      ...
      <button
        ...
        className={`absolute top-2.5 right-2.5 z-10 w-8 h-8 rounded-full border flex items-center justify-center text-sm transition-all shadow-sm
          ${saved
            ...
            : "bg-[#e2e8f0]/80 border-[#94a3b8]/60 text-[#64748b] hover:text-red-400 hover:border-red-400/60 backdrop-blur-sm"
          }`}
      >
        {saved ? "♥" : "♡"}
      </button>
      ...
      <a href={p.url} target="_blank" rel="sponsored noopener noreferrer" className="flex flex-col flex-1">
        ...
        {/* Image */}
        <div className="relative bg-[#e2e8f0]/60 overflow-hidden" style={{ aspectRatio: "1" }}>
          {hasImg ? (
            ...
            <div className="w-full h-full flex flex-col items-center justify-center gap-2">
              ...
              <span className="text-[10px] font-semibold text-[#64748b] text-center px-3 leading-tight">{merchant}</span>
            </div>
          )}

          {/* Badge discount MARE — RetailMeNot style */}
          {hasDiscount && (
            <div className="absolute top-2.5 left-2.5 bg-gradient-to-br from-[#34d399] to-[#14b8a6] text-[#ffffff] font-black rounded-lg px-2.5 py-1 shadow-lg shadow-slate-300/60 leading-none">
        ...
        <div className="p-3 flex flex-col flex-1">
          ...
          <div className="flex items-center gap-1.5 mb-2">
            ...
            <span className="text-[10px] text-[#475569] font-medium truncate">{merchant}</span>
          ...
          {/* Title */}
          <p className="text-xs font-semibold text-[#1e293b] line-clamp-2 flex-1 group-hover:text-[#0f172a] transition-colors leading-snug mb-3">
            {p.title}
          </p>
          ...
          <div className="mb-3">
            {hasOldPrice && (
              <div className="flex items-center gap-2 mb-0.5">
                <span className="text-[11px] text-[#64748b] line-through">{p.old_price!.toFixed(2)} lei</span>
                {hasDiscount && (
...
function DealCard({ m, rank }: { m: Magazin; rank?: number }) {
  ...
  return (
    <a href={link} target="_blank" rel="sponsored noopener noreferrer"
      className="group bg-[#ffffff] border border-[#cbd5e1] hover:border-[#14b8a6]/50 rounded-xl overflow-hidden hover:shadow-xl hover:-translate-y-1 transition-all duration-200 flex flex-col">
      {/* Colored top bar */}
      <div className="h-1 bg-gradient-to-r from-[#0d9488] to-[#14b8a6]"/>
      <div className="p-4 flex-1 flex flex-col">
        {/* Logo + rank */}
        <div className="flex items-start justify-between mb-3">
          <div className="w-12 h-12 rounded-xl border border-[#cbd5e1] bg-[#e2e8f0] flex items-center justify-center overflow-hidden">
            {m.logo_url && imgOk ? (
              // eslint-disable-next-line @next/next/no-img-element
              ...
              <div className={`w-full h-full ${logoBg(name)} flex items-center justify-center rounded-xl`}>
                <span className="text-[#0f172a] font-black text-lg">{initial}</span>
              </div>
            )}
          </div>
        ...
        {/* Promo */}
        {promo && (
          <p className="text-xs text-[#475569] mt-2 line-clamp-2 leading-relaxed flex-1">{promo.nume}</p>
        )}
        {/* Trust row */}
        <div className="flex items-center gap-2 mt-3 pt-3 border-t border-[#cbd5e1]">
          ...
          <span className="text-[10px] text-[#475569]">{m.promotii.length} {m.promotii.length === 1 ? "oferta" : "oferte"}</span>
        </div>
      </div>
      {/* CTA */}
...
export default function ProduseClient({
  ...
  return (
    <div className="min-h-screen bg-[#F7F9FC]">
      ...
      <section className="relative bg-[#F7F9FC] text-[#0f172a] overflow-hidden">
        ...
        <div className="relative max-w-5xl mx-auto px-4 pt-14 pb-16 text-center">
          ...
          <h1 className="text-4xl md:text-6xl font-black tracking-tight leading-tight mb-4">
            <span className="text-[#0f172a]">Cumpara mai inteligent</span><br/>
      ...
      {/* ─── TABS ─────────────────────────────────────────────────────────── */}
      <div className="bg-[#ffffff] border-b border-[#e2e8f0] sticky top-[64px] z-40">
      ...
      {activeTab === "oferte" && (
        <section id="top-deals" className="max-w-7xl mx-auto px-4 py-10">
          <div className="mb-8">
            ...
            <h2 className="text-3xl font-black tracking-tight text-[#0f172a]">Top Deals din Romania</h2>
          ...
          {cuPromotii.length === 0 ? (
            <div className="text-center py-16 bg-[#ffffff] rounded-xl border border-[#cbd5e1]">
              ...
              <p className="font-black text-[#1e293b] mb-2">Se incarca ofertele</p>
              <p className="text-[#475569] text-sm">Datele se actualizeaza zilnic la 08:00</p>
            </div>
          ) : (
            <>
              {/* Top 3 featured */}
              ...
              {cuPromotii.length > 54 && (
                <div className="text-center mt-8">
                  <Link href="/" className="border-2 border-[#cbd5e1] hover:border-[#14b8a6] text-[#475569] hover:text-[#0d9488] font-bold px-8 py-3 rounded-xl text-sm transition-all hover:shadow-md inline-block">
      ...
      {activeTab === "campanii" && (
        <section className="max-w-7xl mx-auto px-4 py-10">
          <div className="mb-8">
            ...
            <h2 className="text-3xl font-black tracking-tight text-[#0f172a]">Bannere & Campanii Active</h2>
            <p className="text-[#475569] text-sm mt-1.5">
          ...
          {bannereValide.length === 0 ? (
            <div className="text-center py-16 bg-[#ffffff] rounded-xl border border-[#cbd5e1]">
              ...
              <p className="font-black text-[#1e293b] mb-2">Nicio campanie activa momentan</p>
              <p className="text-[#475569] text-sm">Bannere se actualizeaza zilnic odata ce magazinele activeaza campanii</p>
            </div>
          ) : (
            <>
              ...
              {bannereValide.slice(0, 1).map(b => (
                <a key={b.id} href={b.landing_url || b.landing_raw} target="_blank" rel="sponsored noopener noreferrer"
                  className="group relative block rounded-xl overflow-hidden border border-[#cbd5e1] hover:border-[#14b8a6]/50 hover:shadow-xl transition-all mb-6">
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-black/50 to-transparent flex items-end p-6">
                    <div>
                      <p className="text-[#0f172a] font-black text-xl">{b.merchant}</p>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                {bannereValide.slice(1).map((b, i) => (
                  <a key={b.id || i} href={b.landing_url || b.landing_raw} target="_blank" rel="sponsored noopener noreferrer"
                    ...
                    className="group relative block rounded-xl overflow-hidden border border-[#cbd5e1] hover:border-[#14b8a6]/50 hover:shadow-lg transition-all">
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    ...
                    <div className="absolute inset-0 bg-gradient-to-t from-black/50 to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex items-end p-3">
                      <span className="text-[#0f172a] text-xs font-bold">{b.merchant}</span>
                    </div>
                  </a>
                ))}
              </div>

              <div className="mt-6 text-center text-xs text-[#64748b] bg-[#ffffff] rounded-xl py-3 px-4">
        ...
        <section id="produse-feed" className="max-w-7xl mx-auto px-4 py-10">
          <div className="flex items-end justify-between mb-6">
            <div>
              ...
              <h2 className="text-3xl font-black tracking-tight text-[#0f172a]">Catalog Produse</h2>
              <p className="text-[#475569] text-sm mt-1.5">
          ...
          {products.length > 0 && (() => {
            ...
            return catList.length > 0 ? (
              <div className="mb-8">
                <p className="text-xs font-bold text-[#64748b] uppercase tracking-widest mb-3">NAVIGATIE PE CATEGORII</p>
                <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-7 gap-2">
                  {catList.map(([slug, cm]) => (
                    <Link key={slug} href={`/produse/${slug}`}
                      className="group bg-[#ffffff] border border-[#cbd5e1] hover:border-[#14b8a6]/60 rounded-xl p-2.5 text-center hover:-translate-y-0.5 transition-all duration-200">
                      <div className="text-xl mb-1">{cm.emoji}</div>
                      <p className="text-[10px] font-bold text-[#334155] group-hover:text-[#0d9488] transition-colors leading-tight">{cm.label}</p>
                      <p className="text-[10px] text-[#64748b] mt-0.5">{(catCounts[slug] || 0).toLocaleString()} produse</p>
                    </Link>
                  ))}
                </div>
              </div>
          ...
          {products.length === 0 ? (
            <div className="text-center py-16 bg-[#ffffff] rounded-xl border border-[#cbd5e1]">
              ...
              <h3 className="font-black text-[#1e293b] text-xl mb-2">Feed-urile se populeaza</h3>
            ...
            <>
              ...
              <div className="bg-[#ffffff] rounded-xl border border-[#cbd5e1] p-4 mb-6">
                <div className="flex flex-wrap gap-3 items-end">
                  ...
                  <div className="flex flex-col gap-1 flex-1 min-w-[200px]">
                    <label className="text-[10px] font-bold text-[#475569] uppercase tracking-widest">Cauta produs</label>
                    <div className="relative">
                      ...
                      <input type="text" placeholder="Titlu, brand, magazin..."
                        ...
                        className="w-full bg-[#e2e8f0] border border-[#cbd5e1] text-[#1e293b] placeholder:text-[#64748b] rounded-xl pl-9 pr-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#14b8a6] transition-all"/>
                    </div>
                  </div>
                  {/* Categorie */}
                  <div className="flex flex-col gap-1 min-w-[160px]">
                    <label className="text-[10px] font-bold text-[#475569] uppercase tracking-widest">Categorie</label>
                    ...
                    <div className="flex gap-1">
                      {[0, 10, 25, 50].map(v => (
                        <button key={v} onClick={() => setMinDiscount(v)}
                          className={`px-3 py-2 rounded-xl text-sm font-semibold transition-colors ${minDiscount === v ? "bg-[#0d9488] text-white" : "border border-[#cbd5e1] bg-[#e2e8f0] text-[#334155] hover:border-[#14b8a6]"}`}>
                          {v === 0 ? "Toate" : `≥${v}%`}
                        </button>
                      ))}
                    </div>
              ...
              <div className="flex items-center justify-between mb-4">
                <p className="text-sm text-[#475569]">
                  <span className="font-bold text-[#1e293b]">{filtrate.length.toLocaleString()}</span> produse gasite
                </p>
                {statsProds.cuImag > 0 && (
                  <span className="text-xs text-[#475569]">{statsProds.cuImag} cu imagine</span>
                )}
              </div>

              {filtrate.length === 0 ? (
                <div className="text-center py-12 bg-[#ffffff] rounded-xl border border-[#cbd5e1]">
                  <p className="text-3xl mb-3">🔍</p>
                  ...
                  {filtrate.length > limit && (
                    <div className="text-center mt-8">
                      <button onClick={() => setLimit(l => l + 48)}
                        className="bg-[#e2e8f0] border border-[#cbd5e1] hover:border-[#14b8a6] text-[#334155] hover:text-[#0d9488] font-bold px-8 py-3 rounded-xl text-sm transition-all hover:shadow-md">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              ...
              <a key={s.href} href={s.href}
                className="group bg-[#e2e8f0] border border-[#cbd5e1] hover:border-[#14b8a6]/50 rounded-xl p-5 hover:shadow-lg hover:-translate-y-0.5 transition-all duration-200">
                <div className="text-3xl mb-3">{s.emoji}</div>
                <p className="font-black text-[#1e293b] text-sm group-hover:text-[#0d9488] transition-colors">{s.label}</p>
      ...
      {savedToast && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 bg-[#ffffff] text-[#0f172a] px-5 py-3 rounded-xl shadow-2xl flex items-center gap-2 text-sm font-semibold animate-in slide-in-from-bottom-4 duration-300">
