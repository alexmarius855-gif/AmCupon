    ...
    <div className="min-h-screen bg-[#0a0f1a]">
      ...
      <header className="bg-[#111827] border-b border-[#1e293b]">
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center gap-2 text-sm">
          ...
          <span className="text-[#cbd5e1] font-semibold">Idei Cadouri</span>
        </div>
      </header>

      {/* Hero */}
      <div className="relative py-12 px-4 text-center overflow-hidden border-b border-[#1e293b]"
        ...
        <div className="relative max-w-3xl mx-auto">
          ...
          <p className="text-[#cbd5e1] text-base mb-6 max-w-xl mx-auto">
            De la botez la Craciun, de la nasi la mama — gasesti cadoul perfect
            din <span className="text-[#f1f5f9] font-bold">1000+ magazine</span> partenere cu livrare in Romania.
          </p>
          <div className="flex flex-wrap justify-center gap-3 text-sm">
            {[
              ...
              <div key={s.l} className="bg-[#111827]/80 border border-[#334155] rounded-xl px-4 py-2 text-center">
      ...
      <div className="max-w-7xl mx-auto px-4 py-10">
        ...
        <section className="mb-12">
          <div className="flex items-center gap-3 mb-6">
            <h2 className="text-xl font-black text-[#f1f5f9]">Ocazii populare</h2>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
            {populare.map(o => (
              <Link
                key={o.slug}
                href={`/cadouri/${o.slug}`}
                className="group relative rounded-xl overflow-hidden border border-[#1e293b] hover:border-[#475569] transition-all hover:-translate-y-1 hover:shadow-xl"
                style={{ background: `linear-gradient(135deg, ${o.color}15 0%, ${o.color}08 100%)` }}
              >
                <div className="p-5">
                  <div className="text-4xl mb-3">{o.emoji}</div>
                  <h3 className="text-[#f1f5f9] font-black text-base mb-1 group-hover:text-[#0f766e] transition-colors">{o.titlu}</h3>
                  <p className="text-[#94a3b8] text-xs mb-3">{o.desc}</p>
                  <div
        ...
        {/* Alte ocazii */}
        <section className="mb-12">
          <h2 className="text-lg font-black text-[#f1f5f9] mb-4">Toate ocaziile</h2>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
            {altele.map(o => (
              <Link
                key={o.slug}
                href={`/cadouri/${o.slug}`}
                className="group flex flex-col items-center text-center bg-[#111827] border border-[#1e293b] hover:border-[#475569] rounded-xl p-4 transition-all hover:-translate-y-0.5"
              >
                <div className="text-3xl mb-2">{o.emoji}</div>
                <div className="text-[#f1f5f9] text-xs font-bold group-hover:text-[#0f766e] transition-colors">{o.titlu}</div>
        ...
        <section className="mb-12">
          <div className="flex items-center gap-3 mb-6">
            <h2 className="text-xl font-black text-[#f1f5f9]">Cadouri dupa buget</h2>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {[
              ...
              <Link
                key={b.slug}
                href={`/cadouri/${b.slug}`}
                className="group relative rounded-xl border border-[#1e293b] hover:border-[#475569] transition-all hover:-translate-y-1 hover:shadow-xl overflow-hidden"
                style={{ background: `linear-gradient(135deg, ${b.from}15 0%, ${b.to}08 100%)` }}
              >
                <div className="p-5 text-center">
                  <div className="text-4xl mb-3">{b.emoji}</div>
                  <div className="text-[#f1f5f9] font-black text-sm mb-1 group-hover:text-[#0f766e] transition-colors">{b.label}</div>
                  <div className="text-[#94a3b8] text-xs">{b.desc}</div>
                </div>
        ...
        <section className="mb-10">
          <h2 className="text-lg font-black text-[#f1f5f9] mb-4">Sfaturi pentru alegerea cadoului perfect</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {SFATURI.map((s, i) => (
              <div key={i} className="bg-[#111827] border border-[#1e293b] rounded-xl p-4">
                <div className="text-2xl mb-2">{s.emoji}</div>
                <h3 className="text-sm font-bold text-[#f1f5f9] mb-1">{s.titlu}</h3>
        ...
        <section className="border-t border-[#1e293b] pt-8">
          <h2 className="text-base font-bold text-[#f1f5f9] mb-4">Magazine recomandate pentru cadouri</h2>
          <div className="flex flex-wrap gap-2">
            {[
              ...
              <Link
                key={m.href}
                href={m.href}
                className="text-sm text-[#cbd5e1] hover:text-[#f1f5f9] bg-[#111827] border border-[#1e293b] hover:border-[#14b8a6]/40 px-3 py-2 rounded-lg transition-all"
              >
                {m.label}
              </Link>
            ))}
