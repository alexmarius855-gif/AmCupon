    ...
    <>
      ...
      <div className="min-h-screen bg-[#0a0f1a] text-[#f1f5f9]">
        <div className="max-w-4xl mx-auto px-4 py-10">
          ...
          {/* Breadcrumb */}
          <nav className="text-sm text-[#cbd5e1] mb-6 flex items-center gap-2">
            <Link href="/" className="hover:text-[#0d9488]">Acasa</Link>
            <span>/</span>
            <Link href="/comparatii" className="hover:text-[#0d9488]">Comparatii</Link>
            <span>/</span>
            <span className="text-[#f1f5f9]">{c.n1} vs {c.n2}</span>
          </nav>

          {/* Hero */}
          <div className="bg-gradient-to-br from-[#0a0f1a]/40 to-[#111827] border border-[#14b8a6]/20 rounded-xl p-8 mb-8">
            <p className="text-[#0f766e] text-sm font-medium mb-2 uppercase tracking-wider">{c.categorie}</p>
            <h1 className="text-3xl md:text-4xl font-bold text-[#f1f5f9] mb-4">{c.titlu_h1}</h1>
            <p className="text-[#cbd5e1] text-lg leading-relaxed">{c.intro}</p>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4 mb-8">
            {[
              ...
              <div key={i} className="bg-[#111827] border border-[#1e293b] rounded-xl p-5">
                ...
                <h2 className="text-[#f1f5f9] font-bold text-lg mb-3">{side.n}</h2>
                <div className="space-y-2 text-sm mb-4">
                  <div className="flex justify-between">
                    <span className="text-[#cbd5e1]">Promotii active</span>
                    <span className={side.stats.promotii_active > 0 ? "text-[#0d9488] font-semibold" : "text-[#94a3b8]"}>
                      {side.stats.promotii_active > 0 ? `${side.stats.promotii_active} oferte` : "Nicio oferta"}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-[#cbd5e1]">Cashback</span>
                ...
                {side.promo.length > 0 && (
                  <div className="mb-4 space-y-2">
                    {side.promo.map((p, j) => (
                      <div key={j} className="bg-[#1e293b] rounded-lg px-3 py-2">
                        <p className="text-[#f1f5f9] text-xs font-medium line-clamp-2">{p.nume}</p>
                        {p.cod_cupon && (
          ...
          {/* Tabel detaliat */}
          <div className="bg-[#111827] border border-[#1e293b] rounded-xl overflow-hidden mb-8">
            <div className="grid grid-cols-3 bg-[#1e293b] px-4 py-3 text-sm font-semibold text-[#cbd5e1]">
              <span>Criteriu</span>
            ...
            {c.puncte.map((p, i) => (
              <div
                key={i}
                className={`grid grid-cols-3 px-4 py-3 text-sm border-t border-[#1e293b] ${i % 2 === 0 ? "" : "bg-[#111827]/50"}`}
              >
                <span className="text-[#cbd5e1] font-medium">{p.aspect}</span>
                <span className="text-center text-[#f1f5f9]">{p.v1}</span>
                <span className="text-center text-[#f1f5f9]">{p.v2}</span>
              </div>
            ))}
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-4 mb-10">
            <div className="bg-[#0a0f1a]/20 border border-[#14b8a6]/30 rounded-xl p-5">
              <p className="text-[#0f766e] font-bold text-sm mb-2 uppercase tracking-wide">Alege {c.n1} daca...</p>
              <p className="text-[#cbd5e1] text-sm leading-relaxed">{c.verdict_m1}</p>
              <Link
            ...
            <div className="bg-[#0a0f1a]/20 border border-[#14b8a6]/30 rounded-xl p-5">
              ...
              <p className="text-[#cbd5e1] text-sm leading-relaxed">{c.verdict_m2}</p>
              <Link
          ...
          {/* FAQ */}
          <div className="mb-10">
            <h2 className="text-xl font-bold text-[#f1f5f9] mb-5">Intrebari frecvente</h2>
            <div className="space-y-4">
              {c.faq.map((f, i) => (
                <div key={i} className="bg-[#111827] border border-[#1e293b] rounded-xl p-5">
                  <h3 className="text-[#f1f5f9] font-semibold mb-2">{f.q}</h3>
                  <p className="text-[#cbd5e1] text-sm leading-relaxed">{f.a}</p>
                </div>
              ))}
            </div>
          </div>
