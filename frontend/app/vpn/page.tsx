  ...
  return (
    <div className="min-h-screen bg-[#F7F9FC]">
      ...
      <section className="relative bg-[#F7F9FC] border-b border-[#e2e8f0] overflow-hidden">
        ...
        <div className="relative max-w-4xl mx-auto px-4 pt-12 pb-10 text-center">
          <nav className="flex justify-center gap-2 text-xs text-[#64748b] mb-8">
            ...
            <span>/</span>
            <span className="text-[#334155]">Cel mai bun VPN Romania</span>
          </nav>
      ...
      <section className="max-w-5xl mx-auto px-4 py-10">
        <div className="bg-[#ffffff] border border-[#e2e8f0] rounded-xl p-5 mb-8">
          <p className="text-[#334155] text-sm text-center">
            <span className="text-[#0f172a] font-bold">Concluzia scurta:</span> Alege <strong className="text-[#0f766e]">NordVPN</strong> daca vrei cel mai bun si nu te deranjeaza un pret putin mai mare. Alege <strong className="text-[#0f766e]">Surfshark</strong> daca ai multi bani putini sau vrei dispozitive nelimitate.
          </p>
        </div>

        <div className="space-y-6">
          {VPN_LIST.map((vpn) => (
            <div key={vpn.name} className={`bg-[#ffffff] border rounded-xl p-6 transition-all ${vpn.rank === 1 ? "border-[#14b8a6]/40 shadow-lg shadow-[#14b8a6]/10" : "border-[#e2e8f0] hover:border-[#cbd5e1]"}`}>
              {vpn.rank === 1 && (
              ...
              <div className="flex flex-col md:flex-row md:items-start gap-5">
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <h2 className="text-xl font-black text-[#0f172a]">#{vpn.rank} {vpn.name}</h2>
                    <span className={`text-[10px] font-black text-[#0f172a] px-2 py-0.5 rounded-full ${vpn.badgeColor}`}>{vpn.badge}</span>
                  </div>
                  <p className="text-[#475569] text-sm mb-4">{vpn.tagline}</p>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-[#64748b]">Ideal pentru: <span className="text-[#475569]">{vpn.ideal}</span></p>
                    </div>
                  </div>
                </div>
                ...
                <div className="md:w-44 flex flex-col items-center gap-3 shrink-0">
                  <div className="text-center">
                    <div className="text-3xl font-black text-[#0f172a]">{vpn.pret_luna}</div>
                    <div className="text-xs text-[#64748b]">/luna ({vpn.pret_nota})</div>
                    <div className="flex items-center justify-center gap-1 mt-1">
                      <span className="text-yellow-400 text-sm">★</span>
                      <span className="text-[#0f172a] font-bold text-sm">{vpn.rating}</span>
      ...
      <section className="max-w-5xl mx-auto px-4 py-8 border-t border-[#e2e8f0]">
        <h2 className="text-xl font-black text-[#0f172a] mb-2">Alte VPN-uri verificate</h2>
        <p className="text-[#64748b] text-sm mb-5">Optiuni suplimentare, pentru cazuri specifice sau buget mai mic.</p>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {ALTE_VPN.map((v) => (
            <div key={v.name} className="bg-[#ffffff] border border-[#e2e8f0] rounded-xl p-5 flex flex-col gap-3">
              <div className="flex items-center gap-2">
                <span className="text-xl">{v.emoji}</span>
                <span className="font-black text-[#0f172a]">{v.name}</span>
              </div>
              <p className="text-[#475569] text-xs">{v.desc}</p>
              <p className="text-[11px] text-[#64748b]">Ideal pentru: <span className="text-[#475569]">{v.ideal}</span></p>
              <a href={v.url} target="_blank" rel="sponsored noopener noreferrer"
                className="mt-auto bg-[#cbd5e1] hover:bg-[#94a3b8] text-[#0f172a] text-sm font-bold py-2.5 rounded-lg text-center transition-all">
                Incearca {v.name} →
              </a>
            </div>
          ))}
        </div>
        <div className="mt-4 bg-[#ffffff]/60 border border-[#e2e8f0] rounded-xl p-4 flex items-center justify-between gap-4 flex-wrap">
          <p className="text-xs text-[#475569]">
            <strong className="text-[#1e293b]">IPRoyal</strong> — nu e un VPN clasic, ci o retea de proxy rezidential. Util pentru web scraping, verificare reclame sau administrare de conturi multiple, nu pentru streaming general.
          </p>
          <a href={LINK_IPROYAL} target="_blank" rel="sponsored noopener noreferrer"
            className="shrink-0 text-xs font-bold text-[#334155] hover:text-[#0f172a] bg-[#e2e8f0] hover:bg-[#cbd5e1] px-3 py-1.5 rounded-lg transition-all">
            Vezi IPRoyal →
          </a>
        </div>
      </section>
      ...
      {/* Cazuri de utilizare */}
      <section className="max-w-5xl mx-auto px-4 py-8 border-t border-[#e2e8f0]">
        <h2 className="text-2xl font-black text-[#0f172a] mb-6">Pentru ce ai nevoie de VPN?</h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {CAZURI_UTILIZARE.map((c, i) => (
            <div key={i} className="bg-[#ffffff] border border-[#e2e8f0] rounded-xl p-4">
              <div className="text-2xl mb-2">{c.emoji}</div>
              <h3 className="text-sm font-bold text-[#0f172a] mb-1">{c.titlu}</h3>
        ...
        <div className="bg-red-950/30 border border-red-800/40 rounded-xl p-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-sm">
            <div>
              ...
              <p className="text-[#475569]">VPN-urile gratuite sunt gratuite pentru ca iti vand istoricul de browsing advertiseri. Tu esti produsul.</p>
            </div>
            <div>
              <p className="text-red-400 font-bold mb-1">Viteza extrem de mica</p>
              <p className="text-[#475569]">Servere aglomerate, bandwidth limitat la 500MB/zi sau 2GB/luna. Inutile pentru streaming.</p>
            </div>
            <div>
      ...
      <section className="max-w-5xl mx-auto px-4 py-10 border-t border-[#e2e8f0]">
        <h2 className="text-2xl font-black text-[#0f172a] mb-6">Intrebari frecvente despre VPN</h2>
      ...
      <section className="max-w-5xl mx-auto px-4 pb-12">
        <div className="bg-gradient-to-r from-[#14b8a6]/40 to-[#14b8a6]/30 border border-[#14b8a6]/30 rounded-xl p-8 text-center">
          <h2 className="text-2xl font-black text-[#0f172a] mb-3">Gata sa iti protejezi conexiunea?</h2>
          <p className="text-[#475569] mb-6 text-sm max-w-xl mx-auto">Oricare ai alege, ai garantie de ramburs 30 zile. Incearca fara risc — daca nu esti multumit, primesti banii inapoi.</p>
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            ...
            <a href="https://surfshark.com" target="_blank" rel="sponsored noopener noreferrer"
              className="bg-[#cbd5e1] hover:bg-[#94a3b8] text-[#0f172a] font-bold px-8 py-3 rounded-xl transition-all hover:-translate-y-0.5">
