  ...
  return (
    <div className="min-h-screen bg-[#0b0a07]">
      ...
      <section className="relative bg-[#0b0a07] border-b border-[#26211a] overflow-hidden">
        ...
        <div className="relative max-w-4xl mx-auto px-4 pt-12 pb-10 text-center">
          <nav className="flex justify-center gap-2 text-xs text-[#8c8064] mb-8">
            ...
            <span>/</span>
            <span className="text-[#c8bda2]">Cel mai bun VPN Romania</span>
          </nav>
            ...
            <div key={vpn.name} className={`bg-[#15120c] border rounded-2xl p-6 transition-all ${vpn.rank === 1 ? "border-[#c9a63e]/40 shadow-lg shadow-[#c9a63e]/10" : "border-[#26211a] hover:border-[#37301f]"}`}>
              ...
              <div className="flex flex-col md:flex-row md:items-start gap-5">
                <div className="flex-1">
                  ...
                  <p className="text-[#a89a78] text-sm mb-4">{vpn.tagline}</p>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-[#8c8064]">Ideal pentru: <span className="text-[#a89a78]">{vpn.ideal}</span></p>
                    </div>
                  </div>
                </div>
                ...
                <div className="md:w-44 flex flex-col items-center gap-3 shrink-0">
                  <div className="text-center">
                    ...
                    <div className="text-xs text-[#8c8064]">/luna ({vpn.pret_nota})</div>
      ...
      <section className="max-w-5xl mx-auto px-4 py-8 border-t border-[#26211a]">
        ...
        <p className="text-[#8c8064] text-sm mb-5">Optiuni suplimentare, pentru cazuri specifice sau buget mai mic.</p>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {ALTE_VPN.map((v) => (
            <div key={v.name} className="bg-[#15120c] border border-[#26211a] rounded-xl p-5 flex flex-col gap-3">
              <div className="flex items-center gap-2">
                <span className="text-xl">{v.emoji}</span>
                <span className="font-black text-white">{v.name}</span>
              </div>
              <p className="text-[#a89a78] text-xs">{v.desc}</p>
              <p className="text-[11px] text-[#8c8064]">Ideal pentru: <span className="text-[#a89a78]">{v.ideal}</span></p>
              <a href={v.url} target="_blank" rel="sponsored noopener noreferrer"
                className="mt-auto bg-[#37301f] hover:bg-[#473d28] text-white text-sm font-bold py-2.5 rounded-lg text-center transition-all">
                Incearca {v.name} →
              </a>
            </div>
          ))}
        </div>
        <div className="mt-4 bg-[#15120c]/60 border border-[#26211a] rounded-xl p-4 flex items-center justify-between gap-4 flex-wrap">
          <p className="text-xs text-[#a89a78]">
            <strong className="text-[#dcd0b8]">IPRoyal</strong> — nu e un VPN clasic, ci o retea de proxy rezidential. Util pentru web scraping, verificare reclame sau administrare de conturi multiple, nu pentru streaming general.
          </p>
          <a href={LINK_IPROYAL} target="_blank" rel="sponsored noopener noreferrer"
            className="shrink-0 text-xs font-bold text-[#c8bda2] hover:text-white bg-[#26211a] hover:bg-[#37301f] px-3 py-1.5 rounded-lg transition-all">
            Vezi IPRoyal →
          </a>
        </div>
      </section>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {CAZURI_UTILIZARE.map((c, i) => (
            <div key={i} className="bg-[#15120c] border border-[#26211a] rounded-xl p-4">
              ...
              <p className="text-xs text-[#a89a78]">{c.desc}</p>
            </div>
          ))}
        </div>
      </section>
        ...
        <div className="bg-red-950/30 border border-red-800/40 rounded-2xl p-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-sm">
            <div>
              ...
              <p className="text-[#a89a78]">VPN-urile gratuite sunt gratuite pentru ca iti vand istoricul de browsing advertiseri. Tu esti produsul.</p>
            </div>
            <div>
              <p className="text-red-400 font-bold mb-1">Viteza extrem de mica</p>
              <p className="text-[#a89a78]">Servere aglomerate, bandwidth limitat la 500MB/zi sau 2GB/luna. Inutile pentru streaming.</p>
            </div>
            <div>
              <p className="text-red-400 font-bold mb-1">Securitate falsa</p>
              <p className="text-[#a89a78]">Unele VPN-uri "gratuite" injecteaza reclame in paginile web sau contin malware. Mai periculos decat fara VPN.</p>
            </div>
          </div>
          <p className="text-[#a89a78] text-sm mt-4">Un VPN platit bun costa <strong className="text-white">2.39-3€/luna</strong> — mai ieftin decat un Netflix. Nu merita riscul pentru cateva euro.</p>
        </div>
      </section>
        ...
        <div className="bg-gradient-to-r from-[#c9a63e]/40 to-[#c9a63e]/30 border border-[#c9a63e]/30 rounded-2xl p-8 text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            ...
            <a href="https://surfshark.com" target="_blank" rel="sponsored noopener noreferrer"
              className="bg-[#37301f] hover:bg-[#473d28] text-white font-bold px-8 py-3 rounded-xl transition-all hover:-translate-y-0.5">
