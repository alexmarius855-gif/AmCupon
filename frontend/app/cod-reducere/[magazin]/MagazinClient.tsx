  ...
  return (
    <a href={p.url} target="_blank" rel="sponsored noopener noreferrer"
      ...
      <div className="p-3 flex flex-col flex-1">
        ...
        <p className="text-sm font-semibold text-gray-900 line-clamp-2 flex-1 group-hover:text-orange-600 transition-colors leading-snug">{p.title}</p>
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <header className="bg-slate-900 border-b border-slate-800 sticky top-0 z-50">
        <div className="max-w-5xl mx-auto px-4 py-3 flex items-center gap-3">
          ...
          <a href="/toate-magazinele" className="ml-auto text-slate-400 hover:text-white transition-colors" title="Cauta magazin">
          ...
          <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6 pb-6">
            ...
            <div className="flex-1 text-center sm:text-left">
              <div className="flex flex-wrap items-center justify-center sm:justify-start gap-2 mb-1">
                ...
                {m.exclusiv && (
                  <span className="text-xs font-bold bg-orange-500 text-white px-2 py-0.5 rounded-full">Exclusiv</span>
                )}
              </div>
              ...
              <button key={t.id} onClick={() => setTabActiv(t.id)}
                ...
                {t.count > 0 && (
                  <span className={`text-xs px-1.5 py-0.5 rounded-full font-black ${
                    tabActiv === t.id ? "bg-orange-500 text-white" : "bg-slate-700 text-slate-300"
                  }`}>{t.count}</span>
                )}
              </button>
            ))}
      ...
      <div className="max-w-5xl mx-auto px-4 py-8">
        ...
        {tabActiv === "coduri" && (
          <>
            {cuCod.length > 0 ? (
              <section>
                <div className="flex items-center gap-3 mb-5">
                  <h2 className="text-xl font-black text-gray-900">Coduri Reducere {nume} {an}</h2>
                  <span className="text-sm text-gray-400">{cuCod.length} coduri active</span>
                </div>
                <div className="space-y-4">
                  {cuCod.map((promo, idx) => {
                    ...
                    return (
                      <div key={idx} className="bg-white rounded-2xl border border-gray-200 shadow-sm hover:shadow-md transition-shadow p-5">
                        <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
                          <div className="flex-1">
                            <div className="flex flex-wrap items-center gap-2 mb-2">
                              {discount && (
                                <span className="text-sm font-black text-orange-500 bg-orange-50 px-2 py-0.5 rounded-lg">-{discount}</span>
                              )}
                              <span className="text-xs font-bold text-teal-600 bg-teal-50 px-2 py-0.5 rounded-full uppercase tracking-wide">Cod Reducere</span>
                              ...
                              {promo.zile_ramase > 1 && promo.zile_ramase <= 3 && (
                                <span className="text-xs font-bold text-orange-500 bg-orange-50 px-2 py-0.5 rounded-full">Expira in {promo.zile_ramase} zile</span>
                              )}
                              {promo.zile_ramase > 3 && (
                                <span className="text-xs text-gray-400">{promo.zile_ramase} zile ramase</span>
                              )}
                            </div>
                          ...
                          <div className="shrink-0 w-full sm:w-48">
                            {isRevealed ? (
                              ...
                              <button onClick={() => copiazaCod(idx, promo.cod_cupon)}
                                ...
                                <p className="text-xs text-gray-400 mt-0.5 group-hover:text-orange-400">Click pentru cod complet</p>
                              </button>
                            )}
                          </div>
                        </div>
              ...
              <div className="bg-white rounded-2xl border border-gray-200 p-12 text-center">
                ...
                <h3 className="text-lg font-black text-gray-800 mb-2">Niciun cod cupon activ</h3>
                <p className="text-gray-400 text-sm mb-5">Momentan nu avem coduri. Verifica sectiunea Oferte sau viziteaza direct magazinul.</p>
                <div className="flex flex-wrap justify-center gap-3">
                  ...
                  <a href={m.url_afiliat || m.url} target="_blank" rel="sponsored noopener noreferrer"
                    className="bg-white border border-gray-200 text-gray-700 font-bold px-5 py-2.5 rounded-xl text-sm hover:border-orange-400 transition-colors">
                    Viziteaza {nume}
                  </a>
                </div>
              </div>
            ...
            <section className="mt-10">
              <h2 className="text-lg font-black text-gray-900 mb-4">Intrebari frecvente</h2>
              <div className="space-y-2">
                {[
                  ...
                  <details key={i} className="bg-white border border-gray-200 rounded-2xl group">
                    <summary className="px-5 py-4 font-semibold text-gray-900 text-sm cursor-pointer list-none flex items-center justify-between gap-4 hover:text-orange-500 transition-colors">
                      {item.q}
        ...
        {tabActiv === "oferte" && (
          <>
            {faraCodd.length > 0 ? (
              <section>
                <div className="flex items-center gap-3 mb-5">
                  <h2 className="text-xl font-black text-gray-900">Oferte {nume} {an}</h2>
                  <span className="text-sm text-gray-400">{faraCodd.length} oferte active</span>
                </div>
                <div className="space-y-4">
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                          <div className="flex-1">
                            <div className="flex flex-wrap items-center gap-2 mb-2">
                              ...
                              <span className="text-xs font-bold text-blue-600 bg-blue-50 px-2 py-0.5 rounded-full uppercase tracking-wide">Oferta</span>
                              {promo.zile_ramase <= 1 && promo.zile_ramase >= 0 && <CountdownTimer zileRamase={promo.zile_ramase}/>}
                              {promo.zile_ramase > 1 && promo.zile_ramase <= 3 && (
                                <span className="text-xs font-bold text-orange-500 bg-orange-50 px-2 py-0.5 rounded-full">Expira in {promo.zile_ramase} zile</span>
                              )}
              ...
              <div className="bg-white rounded-2xl border border-gray-200 p-12 text-center">
                <div className="text-5xl mb-4">🏷</div>
                <h3 className="text-lg font-black text-gray-800 mb-2">Nicio oferta activa</h3>
                <p className="text-gray-400 text-sm mb-5">Revino curand. Ofertele se actualizeaza zilnic.</p>
                {cuCod.length > 0 && (
            ...
            {produse.length > 0 ? (
              <section>
                <div className="flex items-center justify-between mb-5">
                  ...
                  <a href="/produse" className="text-sm font-semibold text-orange-500 hover:text-orange-600">Toate produsele →</a>
                </div>
              ...
              <div className="bg-white rounded-2xl border border-gray-200 p-12 text-center">
                <div className="text-5xl mb-4">🛍</div>
                <h3 className="text-lg font-black text-gray-800 mb-2">Feed produse indisponibil</h3>
                <p className="text-gray-400 text-sm mb-5">Produsele individuale nu sunt disponibile pentru acest magazin. Viziteaza direct site-ul.</p>
                <a href={m.url_afiliat || m.url} target="_blank" rel="sponsored noopener noreferrer"
                  className="bg-orange-500 text-white font-bold px-5 py-2.5 rounded-xl text-sm hover:bg-orange-600 transition-colors">
                  Viziteaza {nume}
                </a>
        ...
        <div className="mt-8 space-y-3">
          ...
          <div className="bg-orange-50 border border-orange-100 rounded-2xl p-5 flex flex-col sm:flex-row items-center gap-4">
        ...
        {similare.length > 0 && (
          <section className="mt-10">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-black text-gray-900">Magazine similare</h2>
              <a href="/toate-magazinele" className="text-sm font-semibold text-orange-500 hover:text-orange-600">Toate →</a>
            </div>
