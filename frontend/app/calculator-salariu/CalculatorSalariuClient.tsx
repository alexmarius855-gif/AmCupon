    ...
    <div className="min-h-screen bg-[#0a0f1a]">
      <div className="max-w-2xl mx-auto px-4 py-10">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-black text-[#f1f5f9] mb-2">Calculator Salariu Net 2026</h1>
          <p className="text-[#cbd5e1]">Afla salariul net din brut, conform cotelor fiscale actuale din Romania</p>
        </div>

        {/* Input card */}
        <div className="bg-[#111827] rounded-xl border border-[#1e293b] p-6 space-y-5">
          <div>
            <label className="text-sm text-[#cbd5e1] mb-1.5 block">Salariul brut (lei/luna)</label>
            <input
              type="number" value={brutStr} onChange={e => setBrutStr(e.target.value)}
              placeholder={`ex: ${salariuMinim}`} min="0"
              className="w-full bg-[#1e293b] border border-[#334155] text-[#f1f5f9] rounded-xl px-4 py-3 text-lg focus:outline-none focus:ring-2 focus:ring-[#14b8a6]/40 focus:border-[#14b8a6]/50"
            />
          </div>

          <div>
            <label className="text-sm text-[#cbd5e1] mb-1.5 block">Persoane in intretinere</label>
          ...
          {r && (
            <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-xl p-5 space-y-2">
              <div className="text-center pb-2">
                <p className="text-[#cbd5e1] text-sm mb-1">Salariu net</p>
                <p className="text-emerald-400 font-black text-4xl">{fmt(r.net)} lei</p>
              </div>
              <div className="h-px bg-[#334155] my-2" />
              <div className="flex justify-between text-sm">
                <span className="text-[#cbd5e1]">Salariu brut</span>
                <span className="text-[#f1f5f9] font-semibold">{fmt(r.brut)} lei</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-[#cbd5e1]">CAS (pensie) -25%</span>
                <span className="text-red-400 font-semibold">-{fmt(r.cas)} lei</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-[#cbd5e1]">CASS (sanatate) -10%</span>
                <span className="text-red-400 font-semibold">-{fmt(r.cass)} lei</span>
              </div>
              {r.deducere > 0 && (
                <div className="flex justify-between text-xs text-[#94a3b8] italic">
                  <span>din care deducere personala ({persoane === 4 ? "4+" : persoane} pers.)</span>
                  <span>-{fmt(r.deducere)} lei din baza impozabila</span>
                </div>
              )}
              <div className="flex justify-between text-sm">
                <span className="text-[#cbd5e1]">Impozit pe venit -10%</span>
                <span className="text-red-400 font-semibold">-{fmt(r.impozit)} lei</span>
              </div>
              <div className="h-px bg-[#334155] my-2" />
              <div className="flex justify-between">
                <span className="text-[#f1f5f9] font-bold text-lg">Total retineri</span>
        ...
        {/* Disclaimer */}
        <p className="text-[#94a3b8] text-xs text-center mt-4 px-4 leading-relaxed">
        ...
        {/* CTA investitii */}
        <div className="bg-gradient-to-r from-[#111827]/30 to-[#111827] border border-[#1e293b]/30 rounded-xl p-6 mt-6 text-center">
          <p className="text-[#f1f5f9] font-bold mb-1">Vrei sa pui surplusul la treaba?</p>
          <p className="text-[#cbd5e1] text-sm mb-4">Compara platformele de investitii si trading disponibile in Romania.</p>
          <Link href="/trading"
            className="inline-block bg-[#0d9488] hover:bg-[#14b8a6] text-white font-bold px-5 py-2.5 rounded-xl text-sm transition-all">
            Vezi platformele →
          </Link>
        </div>
