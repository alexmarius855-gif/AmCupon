  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-[#0b0a07]">
        ...
        <section className="relative bg-[#0b0a07] border-b border-[#26211a] overflow-hidden">
          ...
          <div className="relative max-w-5xl mx-auto px-4 pt-12 pb-10 text-center">
            <nav className="flex justify-center gap-2 text-xs text-[#8c8064] mb-8">
              ...
              <span>/</span>
              <span className="text-[#c8bda2]">Piese Auto</span>
            </nav>
            <div className="text-5xl mb-4">🔧</div>
            <h1 className="text-4xl md:text-5xl font-black text-white mb-4">
              Piese Auto cu <span className="text-transparent bg-clip-text" style={{ backgroundImage: "linear-gradient(135deg, #b8912e, #b8912e)" }}>Reducere</span> {an}
            </h1>
            ...
            <div className="grid grid-cols-3 gap-4 max-w-md mx-auto">
              {[
                ...
                <div key={s.label} className="bg-[#15120c] border border-[#26211a] rounded-2xl py-3 px-2">
                  <div className="text-xl font-black text-white">{s.val}</div>
                  <div className="text-xs text-[#8c8064]">{s.label}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        {/* SUBCATEGORII */}
        <section className="border-b border-[#26211a] py-5 px-4">
          <div className="max-w-5xl mx-auto flex flex-wrap gap-2 justify-center">
            {SUBCATEGORII.map(c => (
              <span key={c.label} className="flex items-center gap-1.5 bg-[#15120c] text-[#c8bda2] font-semibold text-sm px-4 py-2 rounded-full border border-[#26211a]">
        ...
        <div className="max-w-5xl mx-auto px-4 py-8">
                ...
                {cuPromo.map(m => {
                  ...
                  return (
                    <a key={m.magazin} href={`/cod-reducere/${m.magazin}`}
                      className="group bg-[#15120c] border border-[#26211a] hover:border-[#c9a63e]/40 rounded-2xl p-5 transition-all">
                      <div className="flex items-center gap-3 mb-3">
                        {m.logo_url ? (
                          ...
                          <div className="w-10 h-10 rounded-xl bg-[#b8912e] flex items-center justify-center text-white font-black text-lg shrink-0">
                            {nume[0]}
                          </div>
                        )}
                        <div>
                          <p className="font-bold text-white text-sm">{nume}</p>
                          {m.cod_cupon && <span className="text-xs text-[#d8c091] font-bold">COD REDUCERE</span>}
                        </div>
                      </div>
                  ...
                  return (
                    <a key={m.magazin} href={`/cod-reducere/${m.magazin}`}
                      ...
                      {m.logo_url ? (
                        ...
                        <div className="w-8 h-8 rounded-lg bg-[#b8912e] flex items-center justify-center text-white font-black text-sm shrink-0">
                          {nume[0]}
                        </div>
                      )}
                      <span className="text-sm font-semibold text-[#c8bda2] group-hover:text-[#d8c091] transition-colors truncate">{nume}</span>
                    </a>
                  );
                })}
              </div>
          ...
          <section className="mt-10 bg-[#15120c] border border-[#26211a] rounded-2xl p-6">
            ...
            <ul className="space-y-2 text-sm text-[#a89a78]">
              <li><strong className="text-[#dcd0b8]">Compara pretul</strong> intre cel putin 2-3 magazine — diferentele pot fi de 15-30% pentru aceeasi piesa</li>
              <li><strong className="text-[#dcd0b8]">Verifica compatibilitatea</strong> cu modelul exact al masinii inainte de comanda</li>
              <li><strong className="text-[#dcd0b8]">Anvelopele si jantele</strong> au cele mai mari reduceri sezoniere — primavara si toamna (schimb sezonier)</li>
              <li><strong className="text-[#dcd0b8]">Foloseste codul de reducere</strong> afisat pe AmCupon.ro — verificat zilnic, fara cod expirat</li>
            </ul>
          </section>
