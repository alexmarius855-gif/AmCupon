  ...
  return (
    <div className="min-h-screen bg-[#06080b]">
              ...
              return (
                <div key={m.magazin} className="bg-[#14181c] border border-[#1f2329] hover:border-[#ddf93c]/30 rounded-xl p-5 flex flex-col gap-3 transition-all">
                  <div className="flex items-start justify-between">
                    <div>
                      <p className="font-black text-[#ffffff]">{m.magazin.split(".")[0].charAt(0).toUpperCase() + m.magazin.split(".")[0].slice(1)}</p>
                      <p className="text-xs text-[#9399a0]">{m.magazin}</p>
                    </div>
                    {m.comision && <span className="text-[#ddf93c] text-xs font-bold">{m.comision.replace(" sale commission","")}</span>}
                  </div>
                  ...
                  {promo.cod_cupon && (
                    <div className="bg-[#1f2329] border border-dashed border-[#3a4048] rounded-lg px-3 py-2 text-center">
                      ...
                      <p className="font-mono font-black text-[#ddf93c] text-sm tracking-wider">{promo.cod_cupon}</p>
                    </div>
                  )}
                  <a href={m.url_afiliat} target="_blank" rel="sponsored noopener noreferrer"
                    className="bg-[#ddf93c] hover:bg-[#ddf93c] text-[#0c1000] text-sm font-bold py-2.5 rounded-lg text-center transition-all">
                    Vezi cursurile →
                  </a>
                </div>
              );
      ...
      <section className="max-w-5xl mx-auto px-4 py-8 border-t border-[#1f2329]">
        ...
        <p className="text-[#c9ced5] text-sm mb-5">Certificate recunoscute mondial — investitie in cariera pe termen lung.</p>
            ...
            <div key={p.name} className="bg-[#14181c] border border-[#1f2329] hover:border-[#ddf93c]/30 rounded-xl p-6 flex flex-col gap-4 transition-all">
              <div>
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className={`text-[10px] font-black text-[#ffffff] px-2 py-0.5 rounded-full ${p.badgeColor}`}>{p.badge}</span>
                </div>
                <p className="text-[#c9ced5] text-xs">{p.tagline}</p>
              </div>
              <div className="flex items-center justify-between text-sm">
                <span className="text-[#ddf93c] font-bold">{p.pret}</span>
              </div>
